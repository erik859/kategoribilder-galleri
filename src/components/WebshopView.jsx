                ...
                <div style={{
                  ...
                  boxShadow: isDragging ? '0 4px 12px rgba(0,0,0,.15)' : 'none'
                          ...
                          <span {...listeners} {...attributes}
                                      ...
                                      title="Dra för att flytta">⠿</span>span>
                  {img
                              ? <img src={img} style={{ width: 26, height: 26, objectFit: 'cover', borderRadius: 4, flexShrink: 0 }} />
                              : <div style={{ width: 26, height: 26, background: '#f0f0f0', borderRadius: 4, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#ccc' }}>?</div>div>
                  }
                  {editing
                              ...
                              : <span style={{ cursor: 'text', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}
                                                onDoubleClick={() => { setVal(l3); setEditing(true) }}>{l3}</span>span>
                  }
                        <button onClick={() => onDelL3(l1, l2, l3)}
                                    ...
                                    onMouseEnter={e => e.target.style.color = '#c44'} onMouseLeave={e => e.target.style.color = '#ddd'}>✕</button>button>
                </div>div>
        </div>div>
      )
}
                                        ...
                                        <div style={{
                                            ...
                                            boxShadow: '0 1px 3px rgba(0,0,0,.08)'
                                                ...
                                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', background: '#f7f9fb' }}
                                                          ...
                                                          <span {...listeners} {...attributes}
                                                                        ...
                                                                        title="Dra för att flytta hela underkategorin"
                                                                        onClick={e => e.stopPropagation()}>⠿</span>span>
                                                  {l2img
                                                  ...
                                                  {editing
                                                                ...
                                                                : <span style={{ flex: 1, fontSize: 13, fontWeight: 600, color: '#1F4E79', cursor: 'text' }}
                                                                                    onDoubleClick={e => { e.stopPropagation(); setVal(l2); setEditing(true) }}>{l2}</span>span>
                                                  }
                                                          <span style={{ fontSize: 10, color: '#aaa', flexShrink: 0 }}>{cats.length}</span>span>
                                                          <button onClick={e => { e.stopPropagation(); onAddL3(l1, l2) }}
                                                                        style={s('#27AE60')} title="Lägg till kategori">＋</button>button>
                                                          <button onClick={e => { e.stopPropagation(); onDelL2(l1, l2) }}
                                                                        style={s('#e8a0a0', '#c44')} title="Ta bort">✕</button>button>
                                                          <span style={{ fontSize: 11, color: '#999', marginLeft: 2 }}>{open ? '▲' : '▼'}</span>span>
                                                </div>div>
                                          ...
                                          {open && (
                                              <div data-l2-drop={`${l1}::${l2}`} style={{ display: 'flex', flexWrap: 'wrap', gap: 6, padding: '8px 10px', minHeight: 44, background: isL3DropTarget ? '#f0f7ff' : 'transparent', transition: 'background .15s' }}>
                                                ...
                                                {filtered.length === 0 && (
                                                              <span style={{ fontSize: 11, color: '#ccc', alignSelf: 'center' }}>— Inga kategorier —</span>span>
                                                          )}
                                              </div>div>
                                                )}
                                        </div>div>
...
export default function WebshopView() {
    // Use new synced actions from store for L3 rename/delete
    ...
    const renameL1 = (old, nv) => { if (nv !== old) { pushUndo(); setWoo(woo.map(r => r[0]===old?[nv,r[1],r[2]]:r)); saveState() } }
        const renameL2 = (old, nv) => { if (nv !== old) { pushUndo(); setWoo(woo.map(r => r[1]===old?[r[0],nv,r[2]]:r)); saveState() } }
            ...
            // L3 rename: syncs gallery card names too
    const renameL3 = (old, nv) => {
          if (nv === old) return
                pushUndo()
                      renameWooL3(old, nv)
                            saveState()
        ...
        const delL1 = (l1) => { const n=woo.filter(r=>r[0]===l1).length; if (!confirm(`Ta bort hela huvudkategorin "${l1}" med alla underkategorier (${n} rader)?`)) return; pushUndo(); setWoo(woo.filter(r=>r[0]!==l1)); saveState() }
            const delL2 = (l1,l2) => { if (!confirm(`Ta bort "${l2}"?`)) return; pushUndo(); setWoo(woo.filter(r=>!(r[0]===l1&&r[1]===l2))); saveState() }
                ...
                // L3 delete: clears image from gallery card too
    const delL3 = (l1, l2, l3) => {
          if (!confirm(`Ta bort "${l3}"?`)) return
                pushUndo()
                              ...
                              const fi = l1Keys.indexOf(ad.l1), ti = l1Keys.indexOf(od.l1)
                                      if (fi === ti) return
                                              const reordered = arrayMove(l1Keys, fi, ti)
                                                      pushUndo()
                                                              const newWoo = reordered.flatMap(l1 => woo.filter(r => r[0] === l1))
                                                                      setWoo(newWoo); saveState()
                                                                              return
                      }
      
          if (ad?.type === 'l2') {
                  const fromL1 = ad.l1, l2name = ad.l2
                          const toL1 = od?.l1 || fromL1
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
                  {[['all','Alla'],['miss','Saknar bild'],['has','Har bild']].map(([f,label]) => (
                      <button key={f} onClick={() => setWsFilter(f)}
                        ...
                        <span style={{ fontSize:11, color:'#aaa', alignSelf:'center', marginLeft:8 }}>Dra ⠿ för att flytta • Dubbelklicka namn för att redigera</span>span>
                </div>div>
                                                                                                              ...
                                                                                                              const isL2DropHere = activeDrag?.type==='l2' && activeDrag.l1!==l1
                                                                                                                            ...
                                                                                                                            return (
                                                                                                                                            <SortableL1 key={l1} id={`l1::${l1}`} l1={l1} isL2DropHere={isL2DropHere}
                                                                                                                                              ...
                                                                                                                                              </SortableL1>SortableL1>
                                                                                                                                          )
                          })}
                        </SortableContext>SortableContext>
                
                        <DragOverlay modifiers={[restrictToWindowEdges]}>
                          {activeDrag?.type==='l1' && <div style={overlayStyle('#1F4E79')}>⠿ {activeDrag.l1}</div>div>}
                          {activeDrag?.type==='l2' && <div style={overlayStyle('#2E75B6')}>⠿ {activeDrag.l2}</div>div>}
                            ...
                            <div style={{
                                ...
                                border: isL2DropHere?'2px dashed #2E75B6':'1px solid #d0dce8', cursor:'pointer'
                                    ...
                                    <span {...listeners} {...attributes}
                                                ...
                                                title="Dra för att flytta huvudkategori" onClick={e=>e.stopPropagation()}>⠿</span>span>
                                    <span style={{fontSize:10,color:'#999',transition:'transform .15s',display:'inline-block',transform:isOpen?'rotate(90deg)':''}}>▶</span>span>
                              {editing
                                          ? <input autoFocus value={val} onChange={e=>setVal(e.target.value)}
                                          ...
                                          : <span style={{flex:1,fontSize:16,fontWeight:'bold',color:'#1F4E79',cursor:'text'}}
                                                            onDoubleClick={e=>{e.stopPropagation();setVal(l1);setEditing(true)}}>{l1}</span>span>
                              }
                                    <span style={{fontSize:11,color:'#666'}}>{has}/{total} bilder</span>span>
                                    <span style={{fontSize:11,background:pct>=80?'#d4edda':pct>=40?'#fff3cd':'#f8d7da',color:pct>=80?'#155724':pct>=40?'#856404':'#721c24',padding:'1px 8px',borderRadius:10}}>{pct}%</span>span>
                                    <button style={{fontSize:10,padding:'1px 7px',border:'1px solid #27AE60',color:'#27AE60',background:'white',borderRadius:3,cursor:'pointer'}}
                                                onClick={e=>{e.stopPropagation();onAddL2()}}>＋ Lägg till</button>button>
                                    <button style={{fontSize:12,padding:'1px 7px',border:'1px solid #e8a0a0',color:'#c44',background:'white',borderRadius:3,cursor:'pointer',flexShrink:0}}
                                                ...
                                                onClick={e=>{e.stopPropagation();onDelL1()}}>🗑</button>button>
                            </div>div>
                        {isOpen && (
