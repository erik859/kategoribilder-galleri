  ...
  return (
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging ? 0.4 : 1, marginBottom: 6 }}>
      <div style={{
        ...
        {/* Header — only ⠿ handle triggers drag */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 10px', background: '#f7f9fb' }}
          ...
          <button onClick={e => { e.stopPropagation(); onAddL3(l1, l2) }}
            style={s('#27AE60')} title="Lägg till kategori">＋</button>
          <button onClick={e => { e.stopPropagation(); onDelL2(l1, l2) }}
            style={s('#e8a0a0', '#c44')} title="Ta bort">✕</button>
          <span style={{ fontSize: 11, color: '#999', marginLeft: 2 }}>{open ? '▲' : '▼'}</span>
        </div>

        {/* L3 pills — drop target for cross-L2 l3 drops */}
        {open && (
...
export default function WebshopView() {
  ...
  const [l3DropTarget, setL3DropTarget] = useState(null) // `l1::l2` being hovered
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 14 }}>
        ...
        <span style={{ fontSize:11, color:'#aaa', alignSelf:'center', marginLeft:8 }}>Dra ⠿ för att flytta • Dubbelklicka namn för att redigera</span>
      </div>
      ...
      <DndContext sensors={sensors} collisionDetection={pointerWithin}
        ...
        <DragOverlay modifiers={[restrictToWindowEdges]}>
          {activeDrag?.type==='l1' && <div style={overlayStyle('#1F4E79')}>⠿ {activeDrag.l1}</div>}
          {activeDrag?.type==='l2' && <div style={overlayStyle('#2E75B6')}>⠿ {activeDrag.l2}</div>}
    ...
    <div ref={setNodeRef} style={{ transform: CSS.Transform.toString(transform), transition, opacity: isDragging?0.5:1, marginBottom:12 }}>
      <div style={{
        ...
        <span {...listeners} {...attributes}
          ...
          title="Dra för att flytta huvudkategori" onClick={e=>e.stopPropagation()}>⠿</span>
